...
class ObstacleDetectionRoutine:
    ...
    def _obstacle_passed(self, ultra_sound_value: int, tracked_distance: int):
        if len(self.ultrasound_sequence) > 0:
            ...
            if self.ultrasound_sequence in [[0, 1, 0], [1, 0]] or tracked_distance >= 60:
                print("Obstacle passed: ", self.ultrasound_sequence)
                if tracked_distance >= 50:
...
class RobotController:
    def __init__(self):
        self.TURNING_SPEED = 30#30
        self.LEFT_CRUISE_SPEED = 28#26
        self.RIGHT_CRUISE_SPEED = 28#26

        # In Centimeters
        self.WHEEL_RADIUS = 44
        self.CUTTER_DIAMETER = 35
...
def adjust_state(controller: RobotController):
    ...
    if deviation <= controller.angle_error_margin:
        ...
        if (time.time() - last_check) >= 5:
            last_check = time.time()
            controller.TURNING_SPEED = max(min_turning_speed, controller.TURNING_SPEED - int(cached_turning_speed * 0.08))
